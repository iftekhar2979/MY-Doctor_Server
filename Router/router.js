...
const deletePost = require('../function/deletingPost');
...
router.get('/appointments', async (req, res) => {
  ...
  try {
    ...
    appointments.forEach((element) => {
      const filteredAppointment = bookedAppointments.filter(
        (item) => item.service === element.name
      );
      const bookedService = filteredAppointment.map((item) => item.time);
      const remainingSlot = element.slots.filter(
        (slot) => !bookedService.includes(slot)
      );
